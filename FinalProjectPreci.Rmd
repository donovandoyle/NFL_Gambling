...
spreads_comb_under <- left_join(spreads_home_favorites, spreads_away_favorites, .id = "source") %>%
...
spreads_comb_fav <- left_join(spreads_home_favorites, spreads_away_favorites, .id = "source") %>%
  mutate(diff = under_score - fav_score) %>%
  filter(diff < spread) %>%
...
spreads_tidy <- left_join(spreads_comb_fav, spreads_comb_under, .id = "source")
```

```{r plots, echo = FALSE}
(cold_plot <- spreads_tidy %>%
  filter(home_fav == TRUE) %>%
  filter(temp < 60) %>%
  mutate(fav_cover = as.numeric(fav_cover)) %>%
  group_by(fav_team) %>%
  ggplot(aes(x = year, y = fav_cover)) + geom_line())
```
