...
spreads <- read_rds("spreads.rds")
(tidy_shiny <- spreads %>%
  mutate(combined = score_home + score_away) %>%
  select(weather_temperature, over_under_line, combined) %>%
  filter(!is.na(over_under_line)) %>%
  filter(!is.na(weather_temperature)) %>%
  filter(weather_temperature > 0) %>%
  ggplot(aes(x = weather_temperature, y = over_under_line)) + geom_point() + geom_smooth(method = "lm"))
